...
import org.starlightfinancial.wechat.constant.EventType;
import org.starlightfinancial.wechat.constant.MsgType;
import org.starlightfinancial.wechat.lang.SimpleParser;
import org.starlightfinancial.wechat.lang.StreamUtils;
import org.starlightfinancial.wechat.param.SignatureParam;
import org.starlightfinancial.wechat.request.WechatRequest;
import org.starlightfinancial.wechat.response.*;
...
public abstract class WechatSupport {
    ...
    private void setPostData(String xmlStr) {
        ...
        try {
            ...
            this.wechatRequest = (WechatRequest) simpleParser.toObj(xmlStr);
        } catch (Exception e) {
    ...
    private String response() {
        String result = null;
        try {
            SimpleParser simpleParser = new SimpleParser(WechatResponse.class);
            //设置
            simpleParser.setCdataNode(WechatResponse.CDATA_TAG);
            result = simpleParser.toXML(wechatResponse);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
